...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    libpcre3 libpcre3-dev libpcap-dev libnet1-dev libyaml-0-2 libyaml-dev \
    zlib1g zlib1g-dev libcap-ng-dev libcap-ng0 libmagic-dev \
    libjansson-dev libjansson4 libnss3-dev libnspr4-dev rustc cargo \
    && rm -rf /var/lib/apt/lists/*
WORKDIR /tmp
RUN wget -q "https://www.openinfosecfoundation.org/download/suricata-5.0.10.tar.gz" \
    && tar -xzf suricata-5.0.10.tar.gz && cd suricata-5.0.10 \
    && ./configure --prefix=/usr --sysconfdir=/etc --localstatedir=/var \
       --enable-profiling --disable-gccmarch-native \
    && make -j$(nproc) && make install && make install-conf && ldconfig

FROM ubuntu:22.04
ENV DEBIAN_FRONTEND=noninteractive TZ=UTC
RUN apt-get update && apt-get install -y --no-install-recommends \
    ca-certificates curl gzip cron python3 python3-pip nginx \
...
RUN mkdir -p /usr/lib/suricata /var/lib/suricata/rules /var/lib/suricata/update && ldconfig

COPY api/requirements.txt /tmp/requirements.txt
RUN pip3 install --no-cache-dir -r /tmp/requirements.txt && rm /tmp/requirements.txt

RUN mkdir -p /var/cache/cve-data /var/data/analysis /var/log/suricata /var/run/suricata \
    && rm -rf /usr/share/nginx/html/* && rm -f /etc/nginx/sites-enabled/default && rm -f /etc/nginx/conf.d/default.conf
